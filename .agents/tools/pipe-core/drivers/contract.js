...
//   归一化为标准错误码（spawn/auth/config/timeout/protocol/schema/unknown + retryable）。
...
//   unknown  —— 无任何可识别信号（不可盲目重试——归 core Leader 决断，spec RS11）
...
const KINDS = {
  ...
  // unknown：无任何可识别信号的结构/协议故障。不得盲目自动重试——
  // 归 core 的 Leader 决断分支做技术判断（spec RS11「未分类问题进入决断」）。
  unknown: false,
};
...
function classify(result) {
  ...
  // 服务器侧失败（HTTP 5xx/429）是明确的瞬态信号 → network（可重试），不落入 unknown/agent。
  if (/\b(?:50\d|429)\b|internal server error|service unavailable|too many requests|bad gateway|gateway timeout/i.test(msg)) return 'network';
  ...
  // 无信号 → 归 unknown（spec RS11：未分类问题进入 Leader 决断，不盲目 agent 重试）
  return 'unknown';
}
