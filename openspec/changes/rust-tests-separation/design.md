...
- `src-tauri/tests/` 下**每个 `*_tests.rs` 是独立测试 crate**，只能经 `app_lib::<module>::<item>` 访问 `pub`/`pub(crate)` 项。`pub(crate)` 在 `app_lib` crate 内可见，供外部测试 crate 复用，但不泄漏为对外公开 API（`docs` 外部调用者仍看不到）。
...
| 文件 | 提 `pub(crate)` 项 | 生产有调用者？ |
|---|---|---|
| `service/rename.rs` | `is_illegal_name`（仅当测试直接引用；若测试全走 `rename_song` 则无需提） | 待开发确认 |
| `service/searcher/crypto.rs` | `aes_cbc_encrypt`、`aes_ecb_encrypt` | 有（weapi/linuxapi 生产路径） |
| `service/searcher/mod.rs` | `search_song_with_sources`、`search_source_with`、`download_cover_with_timeout`、`aggregate`、`to_halfwidth`、`title_match`、`artist_match`、`source_rank` | 均有（mod 内生产路径调用） |
| `service/searcher/netease.rs` | `search_payload`、`lyric_payload`、`is_error_response`、`parse_search_response`、`parse_lyric_response` | 有 |
| `service/searcher/qqmusic.rs` | `is_error_response`、`parse_search_response`、`parse_lyric_response` | 有 |
| `service/searcher/kugou.rs` | `now_millis`、`random_md5_hex`、`signature`、`search_params`、`is_error_response`、`parse_search_response`、`parse_lyric_search`、`parse_lyric_download` | 有 |
| `service/searcher/itunes.rs` | `parse_search_response` | 有 |
| `service/searcher/lrclib.rs` | `parse_search_response`、`parse_lyric_response` | 有 |

> 不做全量提权：仅「被测试直接引用」的私有项提升，其余保持私有。
...
- 核实结论：上表项绝大多数有生产调用者，预计很少触发；`searcher/mod.rs` 的 8 项在 mod 内生产路径均有调用。
- 若确实触发：加 `#[allow(dead_code)]`（`common/mod.rs` 的 module-level allow 已覆盖工具函数）。**勿改为 `pub`**——pub 会静默该 lint 但泄漏 API，违背可见性最小化。
- G7.2 `cargo clippy` 全绿为门禁；若 CI 以 `-D warnings` 跑 clippy，allow 必须到位。

## 关键技术决策（为什么选它）

### D1. 外置 `tests/` 而非保留内联 + 条件编译
「生产代码零 `#[cfg(test)]`」是已批准需求（用户明确要求分离）。`tests/` 独立 crate 是 Rust 官方的集成测试形态，无需任何配置（Cargo 自动编译），且与既有文件 I/O 集成测试天然同目录，工具/约定合一。代价是跨 crate 可见性（`pub(crate)` 提权 + helper 复制）——这是分离的固有成本，用「可见性最小化」约束来控制。

### D2. `pub(crate)` 而非 `pub`
`pub` 会把测试依赖的项变成公开 API（对 `docs` 外部 crate 可见），`pub(crate)` 只对 crate 内可见。测试 crate 与生产 crate 属同一 crate 依赖图内的「crate 内/外」边界：`tests/` 是独立 crate，但 `pub(crate)` 在 `app_lib` 内部可见——实测可被 `tests/` 引用（`app_lib::` 路径）。权衡：保持最小 API 面，未来若想彻底隐藏可再拆内部模块，不在本变更范围。
...
- `pub(crate)` 提权后 `dead_code` 警告（生产无调用者的提权项）——已核实预计很少，`#[allow(dead_code)]` 兜底；`common/mod.rs` 的 module-level allow 已覆盖工具函数。
- 拆分量大（2234 行），按模块分步、每步 `cargo test` 保绿，避免一次性大炸。
