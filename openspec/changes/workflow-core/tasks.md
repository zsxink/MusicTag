...
> 变更域：infra（纯流程/脚本），不派 rust-backend/vue-frontend。依赖顺序：自举补丁 → 核心骨架 → 决断链 → 自适应编排 → 并行 → 命令与脚本 → 旧脚本移除 → 验证。每组内按序勾选；每批完成做 `feat(106): <任务>` 增量提交。

## 1. 自举补丁：旧脚本加 infra 三处（design.md D10）

> ⚠️ **时序**：组 1 是**启动 `/pipe workflow-core` 之前的前置步骤（非流水线内任务）**——须在 main 上先行提交，旧 pipe（Workflow 工具）启动时脚本才带 infra 支持。若漏做，旧 pipe 会把本变更错判为 `both`（enum 无 infra）、或 infra 域在开发段因 `devResults` 为空直接 `failed`（无 dev 分支）。顺序：1.1-1.3 改 → 1.4 提交 main → 再起 `/pipe workflow-core`。
> 本补丁只改旧脚本、随本变更删除（D9），不碰新核心。

- [ ] 1.1 `.claude/workflows/music-tag-run.js` L33：`ARCHITECT_SCHEMA.domain` enum 加 `infra`
- [ ] 1.2 开发段加 infra 分支：`infra`/`docs`/`spec` 域 → 派 1 个 `leader` 流程维护 agent 实现（scope 指 `.agents/`、`.claude/`、`openspec/`、`AGENTS.md`，`DEV_SCHEMA` 不变），避免 `devResults` 为空触发失败守卫
- [ ] 1.3 验证段加 infra 分支：`infra`/`docs`/`spec` 域 → 跑 `node --test` 核心单测 + `run.js --self-check` + `openspec validate`，跳过 cargo/npm（P4 语义）
- [ ] 1.4 提交 `feat(106): infra domain`（**在 main 上**，先于 `/pipe workflow-core` 启动），使旧 pipe 能判本变更为 infra 域并跑完

## 2. 核心骨架（design.md D1、D5）
...
- [ ] 2.3 `drivers/claude.js`：`claude -p --output-format json --json-schema ... --append-system-prompt roles/<role>.md [--cwd] [--permission-mode] [--allowedTools] [--model]` → 解析 `.structured_output`
...
- [ ] 2.6 `state.js`：节点状态机 + 状态文件读写（原子写盘，仓库根锚定 `.agents/runs/<change>/state.json`）+ 缓存键 + dirty 失效 + commit SHA 落地校验
...
- [ ] 5.3 epic 执行器：读 `epic.json` `dependsOn` → 每批就绪子项 ≤3 并行（子进程 `node run.js <item> --cwd <worktree> --repo-root <主仓库>`——派生时注入 `PIPE_CORE_REPO_ROOT`，见 D1）
- [ ] 5.4 epic 并行状态写 `.agents/runs/<epic>/epic-state.json`（worktree 路径、批次、合并顺序）
...
- [ ] 6.5 `.claude/workflows/pipe-preflight.sh`：前置校验改调新核心 `--self-check`
- [ ] 6.6 `.claude/workflows/pipe-epic-preflight.sh`：适配并行状态
...
- [ ] 7.1 删除 `.claude/workflows/music-tag-run.js`（归档 commit 留档，git 历史可回溯，不保留双轨）
