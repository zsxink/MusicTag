...
> 域：frontend + infra；无依赖。顺序执行：设计文档入库 → 生成 icons → 修复回归 + 入库 → 验证（design.md「任务拆分建议」）。零代码改动，不碰 `src-tauri/tauri.conf.json` 与 Rust/前端源码。
...
## 3. 修复 icons 删除回归 + 源图入库（design.md D6）

- [ ] 3.1 `git add src-tauri/icons/`：恢复 git 索引中的 52 个图标文件（与 v1-skeleton `1104eb5` 一致）
- [ ] 3.2 `git add icon/musictag.png`：源图纳入版本控制
- [ ] 3.3 确认无 `D`（deleted）状态图标文件：`git status -- src-tauri/icons/` 干净
- [ ] 3.4 确认 `git check-ignore` 未忽略 `src-tauri/icons/` 与 `icon/musictag.png`（应 exit=1，即未被忽略）
...
- [ ] 4.4 回归卫生：`git ls-files src-tauri/icons/ | wc -l` = 52；`git status` 无 icons 相关 `D`/`??`
